...
#parcial
    def contar_tiposelectrico(self):
            def contar_electrico(root):
                count = 0
                if root is not None:
                    if root.value == 'electrico':
                        count = 1
                    count += contar_electrico(root.left)
                    count += contar_electrico(root.right)
                return count

            return contar_electrico(self.root)
    
    def contar_tiposacero(self):
            def contar_acero(root):
                count = 0
                if root is not None:
                    if root.value == 'acero':
                        count = 1
                    count += contar_acero(root.left)
                    count += contar_acero(root.right)
                return count

            return contar_acero(self.root)

    def inorden_tipos_pokemon(self,tipo):
        def inorden_tipos_pokemon(root):
            if root is not None:
                inorden_tipos_pokemon(root.left)
                if root.value == tipo:
                    name = root.other_values['name']
                    print(name)
                inorden_tipos_pokemon(root.right)

        inorden_tipos_pokemon(self.root)
